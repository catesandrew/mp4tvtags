...
class Episode:
    ...
    def __init__(self, verbose, program, series, episodeNumbers, fileName):
        ...
        if(self.singleEpisode):
            ...
            self.imdbId = getEpisodeSpecificInfo(verbose, program, series, self.episodeNumbers[0], 'imdb_id')
            self.id = getEpisodeSpecificInfo(verbose, program, series, self.episodeNumbers[0], 'id')
        else:
...
def tagFile(opts, program, series, episode):
    ...
    if series.artworkFileName != "":
        addArtwork = " -picture \"%s/%s\"" % (program.dirPath, series.artworkFileName) #the file we downloaded earlier
    else:
        addArtwork = ""
    ...
    addMediaKind = " -type \"TV Show\"" #set type to TV Show
    addArtist = " -artist \"%s\"" % series.seriesName
    addName =  " -song \"%s\"" % episode.episodeName
    ...
    data = episode.overview
    shortDescription = (data[:250] + '...') if len(data) > 250 else data
    addDescription = " -description \"%s\"" % shortDescription  
    
    #addLongDescription = " -longdesc \"%s\"" % episode.longOverview
    data = episode.longOverview
    addLongDescription = " -longdesc \"%s\"" % episode.longOverview
    
    
    addTVNetwork = " -network \"%s\"" % series.network
    addTVShowName = " -show \"%s\"" % series.seriesName
    addTVEpisode = " -episodeid \"%s\"" % episode.productionCode
    addTVSeasonNum = " -season \"%i\"" % series.seasonNumber
    
    #kept for clarity as it same decision is made
    if episode.singleEpisode:
        addTVEpisodeNum = " -episode \"%i\"" % episode.episodeNumbers[0]
        addTracknum = " -track \"%i\"" % episode.episodeNumbers[0]
    else:
        addTVEpisodeNum = " -episode \"%i\"" % episode.episodeNumbers[0]
        addTracknum = " -track \"%i\"" % episode.episodeNumbers[0]    
    #end if episode.singleEpisode
    ...
    addReleaseDate = " -year \"%s\"" % episode.firstAired
    ...
    addComment = " -comment \"{'imdb_id':'%s', 'tvdb_id':'%s'}\"" % (episode.imdbId, episode.id)
    ...
    if len(episode.writers) > 0:
        ...
        addScreenwriters = " -swriters \"%s\"" % screenwriters
    #end if len
    
    #Create the command line string
    tagCmd = "\"%s\" %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s \"%s/%s\"" % \
    (program.mp4tagger, addArtwork.encode("utf-8"), addMediaKind.encode("utf-8"), \
    ...
    addTVEpisode.encode("utf-8"), addTVSeasonNum.encode("utf-8"), addTVEpisodeNum.encode("utf-8"), \
    addRating.encode("utf-8"), addReleaseDate.encode("utf-8"), addComment.encode("utf-8"), \
    addLongDescription.encode("utf-8"), addCast.encode("utf-8"), addDirectors.encode("utf-8"), addScreenwriters.encode("utf-8"), \
    program.dirPath, episode.fileName)
...
def main():
    ...
    parser.set_defaults( interactive=True, debug=False, verbose=True,
    ...
    for fileName in glob.glob("*.mp4") + glob.glob("*.m4v"):
        ...
        if opts.removetags:
            ...
            removeTagsCmd = "\"" + program.mp4tagger + "\" -r AacCdDgGHilmMnNoPsStTywR \"" + program.dirPath + "/" + episode.fileName + "\""
            if opts.debug:
